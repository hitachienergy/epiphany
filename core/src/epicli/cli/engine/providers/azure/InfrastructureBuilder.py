...
class InfrastructureBuilder(Step):
    def __init__(self, docs, existing_subnet_id):
        ...
        self.docs = docs
        self.existing_subnet_id = existing_subnet_id
    ...
    def run(self):
        ...
        # If an existing network is being used, we don't need to create a vnet
        if self.existing_subnet_id is None:
        ...
        for component_key, component_value in self.cluster_model.specification.components.items():
            ...
            if not(self.use_network_security_groups) and (self.use_public_ips and self.existing_subnet_id is None):
            ...
            if self.use_network_security_groups and not(self.existing_subnet_id is None):
                self.logger.warning('Since an existing subnet is being used, the use_network_security_group property will be ignored')
            ...
            # Only create subnet(s) if we aren't using an existing subnet
            if self.existing_subnet_id is None:
            ...
            for index in range(vm_count):
                ...
                # A network security group is only created if we aren't using an existing network
                if self.use_network_security_groups and self.existing_subnet_id is None:
                ...
                if self.existing_subnet_id is None:
                    ...
                    subnet_name = ''
                    existing_subnet_id = self.existing_subnet_id
    ...
    def get_network_interface(self, component_key, component_value, vm_config, subnet_name, existing_subnet_id, security_group_name, public_ip_name, index):
        ...
        network_interface.specification.existing_subnet_id = existing_subnet_id
