...
class AWSConfigBuilder(InfrastructureConfigBuilder):
    def build(self, cluster_model, user_input):
        ...
        for component_key, component_value in cluster_model["specification"]["components"].items():
            ...
            if subnet is None:
                subnet = self.get_subnet(cluster_model, component_value, subnet_index, user_input, vpc_name)
                result.append(subnet)

                security_group = self.get_security_group(cluster_model, subnet, subnet_index, user_input, vpc_name)
                result.append(security_group)

                route_table_association = self.get_route_table_association(cluster_model, user_input, route_table["specification"]["name"], subnet["specification"]["name"], subnet_index)
            ...
            launch_configuration = self.get_launch_configuration(autoscaling_group, cluster_model, component_key, security_group["specification"]["name"], user_input)
    ...
    def get_vpc_config(self, cluster_model, user_input):
        ...
        vpc_config["specification"]["name"] = "aws-vpc-"+cluster_model["specification"]["cloud"]["cluster_name"].lower()
        return vpc_config
    ...
    def get_autoscaling_group(self, cluster_model, component_key, component_value, subnet_name, user_input):
        ...
        autoscaling_group["specification"]["name"] = 'aws-asg-' + cluster_model["specification"]["cloud"][
            "cluster_name"].lower() + '-' + component_key.lower()
    ...
    def get_launch_configuration(self, autoscaling_group, cluster_model, component_key, security_group_name, user_input):
        launch_configuration = self.get_config_or_default(user_input, 'infrastructure/launch-configuration')
        launch_configuration["specification"]["name"] = 'aws-launch-config-' + cluster_model["specification"]["cloud"][
            "cluster_name"].lower() + '-' + component_key.lower()
    ...
    def get_subnet(self, cluster_model, component_value, subnet_index, user_input, vpc_name):
        ...
        subnet["specification"]["name"] = 'aws-subnet-' + cluster_model["specification"]["cloud"][
            "cluster_name"].lower() + '-' + str(subnet_index)
        return subnet

    def get_security_group(self, cluster_model, subnet, subnet_index, user_input, vpc_name):
        security_group = self.get_config_or_default(user_input, 'infrastructure/security-group')
        security_group["specification"]["name"] = 'aws-security-group-' + cluster_model["specification"]["cloud"][
            "cluster_name"].lower() + '-' + str(subnet_index)
    ...
    def get_route_table_association(self, cluster_model, user_input, route_table_name, subnet_name, subnet_index):
    ...
    def get_internet_gateway(self, cluster_model, user_input, vpc_name):
    ...
    def get_routing_table(self, cluster_model, user_input, vpc_name, internet_gateway_name):
