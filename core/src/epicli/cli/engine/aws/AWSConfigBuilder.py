...
class AWSConfigBuilder(InfrastructureConfigBuilder):
    def build(self, cluster_model, user_input):
        ...
        for component_key, component_value in cluster_model["specification"]["components"].items():
            ...
            if subnet is None:
                subnet = self.get_subnet(cluster_model, component_value, subnet_index, user_input, vpc_config)
                result.append(subnet)

                security_group = self.get_security_group(cluster_model, subnet, subnet_index, user_input, vpc_config)
            ...
            autoscaling_group = self.get_autoscaling_group(cluster_model, component_key, component_value, subnet, user_input)

            security_group["specification"]["rules"] += autoscaling_group["specification"]["security"]["rules"]

            launch_configuration = self.get_launch_configuration(autoscaling_group, cluster_model, component_key, security_group, user_input)
    ...
    def get_autoscaling_group(self, cluster_model, component_key, component_value, subnet, user_input):
        ...
        autoscaling_group["specification"]["name"] = 'aws-asg-' + cluster_model["specification"]["cloud"][
            "cluster_name"].lower() + component_key
    ...
    def get_launch_configuration(self, autoscaling_group, cluster_model, component_key, security_group, user_input):
    ...
    def get_subnet(self, cluster_model, component_value, subnet_index, user_input, vpc_config):
    ...
    def get_security_group(self, cluster_model, subnet, subnet_index, user_input, vpc_config):
