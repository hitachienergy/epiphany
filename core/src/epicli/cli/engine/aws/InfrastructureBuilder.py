...
class InfrastructureBuilder(Step):
    ...
    def run(self, cluster_model, user_input):
        ...
        for component_key, component_value in cluster_model.specification.components.items():
            if component_value['count'] < 1:
                continue
            subnet = select_first(result, lambda item: item.kind == 'infrastructure/subnet' and
                                  item.specification.cidr_block == component_value.subnet_address_pool)
            security_group = select_first(result, lambda item: item.kind == 'infrastructure/security-group' and
                                          item.specification.cidr_block == component_value.subnet_address_pool)
            ...
            if subnet is None:
                ...
                route_table_association = self.get_route_table_association(cluster_name, user_input,
                                                                           route_table.specification.name,
                                                                           subnet.specification.name, subnet_index)
                result.append(route_table_association)

                subnet_index += 1

            autoscaling_group = self.get_autoscaling_group(cluster_model, component_key, component_value,
                                                           subnet.specification.name, user_input)

            security_group.specification.rules += autoscaling_group.specification.security.rules

            launch_configuration = self.get_launch_configuration(autoscaling_group, cluster_name, component_key,
                                                                 security_group.specification.name, user_input)

            self.set_image_id_for_launch_configuration(cluster_model, user_input, launch_configuration,
                                                       autoscaling_group)
    ...
    def get_autoscaling_group(self, cluster_model, component_key, component_value, subnet_name, user_input):
        ...
        autoscaling_group.specification.name = 'aws-asg-' + cluster_name.lower() + '-' + component_key.lower()
    ...
    def get_launch_configuration(self, autoscaling_group, cluster_name, component_key, security_group_name, user_input):
        ...
        launch_configuration.specification.name = 'aws-launch-config-' + cluster_name.lower() + '-' \
                                                  + component_key.lower()
    ...
    def get_subnet(self, cluster_name, component_value, subnet_index, user_input, vpc_name):
        ...
        subnet.specification.name = 'aws-subnet-' + cluster_name.lower() + '-' + str(subnet_index)
        return subnet
    ...
    def get_security_group(self, cluster_name, subnet, subnet_index, user_input, vpc_name):
        ...
        security_group.specification.name = 'aws-security-group-' + cluster_name.lower() + '-' + str(subnet_index)
    ...
    def get_route_table_association(self, cluster_name, user_input, route_table_name, subnet_name, subnet_index):
        ...
        route_table_association.specification.name = 'aws-route-association-' + cluster_name.lower() + '-' + str(subnet_index)
    ...
    def get_internet_gateway(self, cluster_name, user_input, vpc_name):
        ...
        internet_gateway.specification.name = 'aws-internet-gateway-' + cluster_name.lower()
    ...
    def get_routing_table(self, cluster_name, user_input, vpc_name, internet_gateway_name):
        ...
        route_table.specification.name = 'aws-route-table-' + cluster_name.lower()
    ...
    def get_virtual_machine(component_value, cluster_model, user_input):
        ...
        model_with_defaults = select_first(user_input, lambda x: x.kind == 'infrastructure/virtual-machine' and
