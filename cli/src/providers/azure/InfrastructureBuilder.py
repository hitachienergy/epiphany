...
class InfrastructureBuilder(Step):
    ...
    def get_vm(self, component_key, vm_config, availability_set, network_interface_name, security_group_association_name, index):
        ...
        if component_key in ['kubernetes_master', 'kubernetes_node'] and len(vm.specification.hostname) > 63:
            raise Exception(f'Kubernetes node name part must be no more than 63 characters, yours is {vm.specification.hostname}. Consider setting alt_component_name property.')
