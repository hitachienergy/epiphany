...
def callRabbitMQDeploymentTests
  ...
  rabbitmq_http_port = 15_672
  user = 'testuser' + SecureRandom.hex(5)
  pass = SecureRandom.hex

  service_port = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
                   i['name'] == 'rabbitmq'
                 end ['service']['port']
  service_management_port = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
                              i['name'] == 'rabbitmq'
                            end ['service']['management_port']
  service_replicas = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
                       i['name'] == 'rabbitmq'
                     end ['service']['replicas']
  service_namespace = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
                        i['name'] == 'rabbitmq'
                      end ['service']['namespace']
  service_name = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
                   i['name'] == 'rabbitmq'
                 end ['service']['name']
  plugins = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
              i['name'] == 'rabbitmq'
            end            ['rabbitmq']['plugins']
  version = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
              i['name'] == 'rabbitmq'
            end            ['image_path'].split(':').last

  unless readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
           i['name'] == 'rabbitmq'
         end ['rabbitmq']['amqp_port'].nil?
    rabbitmq_amqp_port = readDataYaml('configuration/applications')['specification']['applications'].detect do |i|
                           i['name'] == 'rabbitmq'
                         end ['rabbitmq']['amqp_port']
  end
