...
- kubernetes_master: When increased this will setup additional control plane nodes, but in the case of non-ha k8s cluster, existing control plane node must be promoted first. At the moment there is [no ability](../../../../../issues/1579) to downscale.
- kubernetes_node: When increased this will setup additional nodes with `kubernetes_master`. There is [no ability](../../../../../issues/1580) to downscale.
- ignite
- kafka: When changed this will setup or remove additional nodes for the Kafka cluster. Note that there is an [issue](../../../../../issues/1576) that needs to be fixed before scaling usage.
- load_balancer
- logging: Sometimes it works, but often there is an [issue](../../../../../issues/1575) with Kibana installation that needs to be resoved
- monitoring
...
- postgresql: At the moment does not work correctly, there is an [issue](../../../../../issues/1577). When changed this will setup or remove additional nodes for Postgresql. Note that extra nodes can only be setup to do replication by adding the following additional `configuration/postgresql` configuration:
