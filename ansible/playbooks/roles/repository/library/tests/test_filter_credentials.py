...
from repository.library.filter_credentials import _get_filtered_manifest

from repository.library.tests.data.filter_credentials_data import (
    CLUSTER_DOC_ANY,
    CLUSTER_DOC_AWS,
    CLUSTER_DOC_AZURE,
...
def test_epiphany_cluster_doc_filtering(CLUSTER_DOC, EXPECTED_OUTPUT_DOC, mocker):
    ...
    mocker.patch('repository.library.filter_credentials.yaml.safe_load_all', return_value=[deepcopy(CLUSTER_DOC)])
    mocker.patch('repository.library.filter_credentials.yaml.dump_all', side_effect=lambda docs: docs)
    mocker.patch('repository.library.filter_credentials.Path.open')
...
def test_not_needed_docs_filtering(mocker):
    # Ignore yaml parsing, work on python objects:
    mocker.patch('repository.library.filter_credentials.yaml.safe_load_all', return_value=deepcopy(MANIFEST_WITH_ADDITIONAL_DOCS))
    mocker.patch('repository.library.filter_credentials.yaml.dump_all', side_effect=lambda docs: docs)
    mocker.patch('repository.library.filter_credentials.Path.open')
